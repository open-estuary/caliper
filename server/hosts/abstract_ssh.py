...
def _make_ssh_cmd_default(user="root", port=22, opts='', hosts_file='/dev/null',
    ...
    base_command = ("/usr/bin/ssh -a -x %s -o StrictHostKeyChecking=no "
                    ...
                    "-o ConnectTimeout=%d -o ServerAliveInterval=%d -l %s -p %d")
...
class AbstractSSHHost(remote_host.RemoteHost):
    ...
    def _initialize(self, hostname, user="root", port=22, password="", *args, **dargs):
        super(AbstractSSHHost, self)._initialize(hostname=hostname, *args, **dargs)
        ...
        control path option. if master-ssh is enabled, these fields will be initialized
        by start_master_ssh when a new SSH connection is initialed.
        """
    ...
    def use_rsync(self):
        ...
        # check if the rsync is available on the remote host. If it's not, don't
        # try to use it for any future file transfers.
        self._use_rsync = self._check_rsync()
        if not self._use_rsync:
            logging.warn("rsync not available on remote host %s -- disabled", self.hostname)
        return self._use_rsync
    ...
    def _make_rsync_cmd(self, sources, dest, delete_dest, preserve_symlinks):
        """
        given a list of file paths, encodes it as a single remote path, in the style used
        by rsync and scp
        """
        ssh_cmd = _make_ssh_cmd_default(user=self.user, port=self.port, opts=self.master_ssh_option,
        ...
        return command % (symlink_flag, delete_flag, ssh_cmd, " ".join(sources), dest)
    ...
    def _make_rsync_compatible_source(self, source, is_local):
        """
        Applies the same logic as _make_rsync_ccompatible_globs, but applies if to an entire list
        of sources, producing a new list of sources, properly quoted.
        """
    ...
    def _make_scp_cmd(self, sources, dest):
        """
        Given a list of source paths and a destination path, produces the appropriate
        scp command for encoding it. Remote paths must be pre-encoded.
        """
        command = ("scp -rq %s -o StrictHostKeyChecking=no -o UserKnownHostsFile=%s -P %d %s '%s'")
        return command % (self.master_ssh_option, self.known_hosts_file,
                            self.port, " ".join(sources), dest)

    def _set_umask_perms(self, dest):
        """
        Given a destination file/dir (recursively) set the permissions on all the
        files and directories to the max allowed by running umask.
        """
        umask = os.umask(0)
        os.umask(umask)
    ...
    def get_file(self, source, dest, delete_dest=False, preserve_perm = True,
        ...
        Args:
            ...
            preserve_perm: tells get_file() to try to preserve the sources permissions
                        on files and dirs
            preserve_symlinks: try to preserver symlinks instead of transforming
                        them into files/dirs on copy
        Raiseds:
        ...
        if not preserve_perm:
            # have no way to tell scp to not try to preserve the permssions so set them
            # after copy instead.
            # for rsync we could use "--no-p --chmod=ugo-rmX" but those options are only
            # in veryrecent rsync versions
            self._set_umask_perms=(dest)

    def send_file(self, source, dest, delete_dest=False, preserve_symlinks=False):
        """
        ...
        if try_scp:
            ...
            if delete_dest:
                ...
                # to be a dir.It therr's a single path being transferred and it is a dir,
                # the destination also has to be a dir
    ...
    def _cleanup_master_ssh(self):
        """
        Release all resources (process, temporary directory) used by an active master SSH connection.
        """
    ...
    def start_master_ssh(self):
        ...
        If the master SSH support is enabled and a master SSH connection is not active
        already, start a new one in the backgroud.
        """   
        ...
        if self.master_ssh_job is not None:
            if self.master_ssh_job.sp.poll() is not None:
                logging.info("Master ssh connection to %s is down.", self.hostname)
                self._cleanup_master_ssh()
