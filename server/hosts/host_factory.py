...
def create_host(hostname, ssh_user, ssh_pass, ssh_port):
    """auto_monitor=True, follow_paths=None, pattern_paths=None,netconsole=False,"""
    ...
    if SSH_ENGINE == 'raw_ssh' :
        classes = [ssh_host.SSHHost, ssh_host.AsyncSSHMixin]  # ssh_host.AsyncSSHMixin
    else:
        raise error.AutoError("Unknown SSH engine %s. Please verify the value of the "
                                "configuration key 'ssh_engine' on the .ini file " % SSH_ENGINE)
