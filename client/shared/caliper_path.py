...
# This means that the caliper is already installed. Only instance can execute as the updatation of the results will happen under ~/home/user/caliper_workspace
...
class Folder(Singleton):
    ...
    def set_up_path(self):
        self.build_dir = os.path.join(CALIPER_REPORT_HOME, self.name, 'caliper_build')
        self.exec_dir = os.path.join(CALIPER_REPORT_HOME, self.name, 'caliper_exec')
        self.results_dir = os.path.join(CALIPER_REPORT_HOME, self.name, 'results')
        self.caliper_log_file = os.path.join(CALIPER_REPORT_HOME, self.name, 'caliper_exe.log')
        self.summary_file = os.path.join(CALIPER_REPORT_HOME, self.name, 'results_summary.log')
