...
def handle_prompts(session, username, password, prompt, timeout=50, debug=False):
    """
    Connect to a remote host (guest) using SSH or Telnet or other else.
    Wait for questions and provide answers. If timeout expires while waiting for output
    from the child  -- fail.
    ...
    while True:
        try:
            match, text = session.read_until_last_line_matches(
                    [r"[Aa]re you sure", r"[Pp]assword:\s*", r"(?<![Ll]ast).*[Ll]ogin:\s*$",
                        r"[Cc]onnection.*closed", r"[Cc]onnection.*refused", r"[Pp]lease wait",
                        r"[Ww]arning", r"[Ee]nter.*username", r"[Ee]nter.*password", prompt],
...
def remote_login(client, host, port, username, password, prompt, linesep="\n",
    ...
    if host and host.lower().startswith("fe800"):
        if not interface:
            raise LoginError("When using ipv6 linklocal an interface must be assigned")
        host = "%s%%%s" % (host, interface)

    if client == "ssh":
        cmd = ("ssh -o UserKnownHostsFile=/dev/null -o PreferredAuthentications=password "
