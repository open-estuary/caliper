...
class BaseCfg(object):
    ...
    def get_value(self, section, key):
        ...
        try:
            ...
            raise ConfigError("Value '%s' not found in section '%s'" % (key, section))
        else:
            return val
...
def get_bench_info(config_file, bench_name):
    ...
    try:
        build_file =  baseCfg.get_value(name, 'build')
        run_config = baseCfg.get_value(name, 'run')
        parser_file = baseCfg.get_value(name, 'parser')
